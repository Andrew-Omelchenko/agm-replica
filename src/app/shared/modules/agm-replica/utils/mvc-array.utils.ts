...
export function createMVCEventObservable<T>(array: google.maps.MVCArray<T>): Observable<IMVCEvent<T>> {
  const eventNames = ['insert_at', 'remove_at', 'set_at'];
  return fromEventPattern(
    (handler) =>
      eventNames.map((eventName) =>
        array.addListener(eventName, (index: number, previous?: T) =>
          handler.apply(array, [{ newArr: array.getArray(), eventName, index, previous } as IMVCEvent<T>]),
        ),
      ),
    (_handler, evListeners: google.maps.MapsEventListener[]) =>
...
export class MvcArrayMock<T> implements google.maps.MVCArray<T> {
  ...
  pop(): T {
    ...
    if (!deleted) {
      throw new Error("Can't pop any value!");
    }
    // @ts-ignore
